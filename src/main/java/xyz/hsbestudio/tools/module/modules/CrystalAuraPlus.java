...
import xyz.hsbestudio.tools.utils.CrystalUtils;
import xyz.hsbestudio.tools.utils.DamageUtilsPlus;
import xyz.hsbestudio.tools.utils.PlayerUtilsPlus;
import xyz.hsbestudio.tools.utils.TimerUtils;
...
public class CrystalAuraPlus extends Module {
    ...
    public CrystalAuraPlus() {
        super(DinnerTools.CATEGORY, "crystal-aura+", "Automatically places and attacks crystals.");
    }
    ...
    private void onEntityAdded(EntityAddedEvent event) {
        ...
        if (fastBreak.get()) {
            ...
            if (smartCheck.get()) {
                if (CrystalUtils.isSurroundHolding() || (slowFacePlace.get() && CrystalUtils.isFacePlacing() || (targetPopInvincibility.get() && CrystalUtils.targetJustPopped())))
                    return;
            }
    ...
    private void doBreak() {
        if (!doBreak.get() || breakTimer > 0 || switchTimer > 0 || attacks >= attackFrequency.get() || (popPause.get() != PopPause.Place && CrystalUtils.targetJustPopped()))
            return;
    ...
    private float getBreakDamage(Entity entity, boolean checkCrystalAge) {
        ...
        if (!CrystalUtils.shouldIgnoreSelfBreakDamage()) {
            float selfDamage = DamageUtilsPlus.crystalDamage(mc.player, entity.getPos(), predictMovement.get(), breakRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private void doPlace() {
        if (!doPlace.get() || placeTimer > 0 || (popPause.get() != PopPause.Break && CrystalUtils.targetJustPopped()))
            return;
        ...
        BlockIterator.register((int) Math.ceil(placeRange.get()), (int) Math.ceil(placeRange.get()), (bp, blockState) -> {
            ...
            if (!CrystalUtils.shouldIgnoreSelfPlaceDamage()) {
                float selfDamage = DamageUtilsPlus.crystalDamage(mc.player, vec3d, predictMovement.get(), placeRange.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
    ...
    private boolean isOutOfBreakRange(Entity entity) {
        ...
        double distance = PlayerUtilsPlus.distanceFromEye(entity);
    ...
    private float getDamageToTargets(Vec3d vec3d, boolean breaking, boolean fast) {
        ...
        if (fast) {
            ...
            if (!(smartDelay.get() && breaking && target.hurtTime > 0))
                damage = DamageUtilsPlus.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
        } else {
            for (PlayerEntity target : targets) {
                if (smartDelay.get() && breaking && target.hurtTime > 0) continue;

                float dmg = DamageUtilsPlus.crystalDamage(target, vec3d, predictMovement.get(), explosionRadiusToTarget.get().floatValue(), ignoreTerrain.get(), fullBlocks.get());
