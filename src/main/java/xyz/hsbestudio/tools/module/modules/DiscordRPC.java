...
public class DiscordRPC extends Module {
    ...
    private final Setting<List<String>> line1Strings = sgLine1.add(new StringListSetting.Builder()
        ...
        .defaultValue(
            "Owning with 06ED",
            "I want to worship 06ED!",
            "Do fisting on {server}"
        )
    ...
    public DiscordRPC() {
        super(DinnerTools.CATEGORY, "RPC", "WTF 06ED in Discord???!!!");
    ...
    private static boolean meteorRpcWasActive = false;
    ...
    public void onActivate() {
        ...
        DiscordIPC.start(1085570225958813766L, null);

        rpc.setStart(System.currentTimeMillis() / 1000L);
        rpc.setLargeImage("dinner_main", "06ED's tools");
    ...
    public void onDeactivate() {
        DiscordPresence presence = Modules.get().get(DiscordPresence.class);
        DiscordIPC.stop();
        if (meteorRpcWasActive && !presence.isActive()) presence.toggle();
    }
    ...
    private void recompile(List<String> messages, List<Script> scripts) {
        ...
        for (int i = 0; i < messages.size(); i++) {
            ...
            if (result.hasErrors()) {
                if (Utils.canUpdate()) MeteorStarscript.printChatError(i, result.errors.get(0));
                continue;
            }
