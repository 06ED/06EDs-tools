...
//    public static PlayerActionC2SPacket.Action faceVectorPacketInstant(Vec3d vec, Boolean roundAngles) {
//        Vec2f rotations = RotationUtil.getRotationTo(vec);
//
//        CPacketPlayer.Rotation e = new CPacketPlayer.Rotation(rotations.x, rotations.y, mc.player.onGround);
//
        ...
        for (int x = (int) Math.floor(box.minX); x < Math.ceil(box.maxX); x++)
            for (int y = (int) Math.floor(box.minY); y < Math.ceil(box.maxY); y++)
                for (int z = (int) Math.floor(box.minZ); z < Math.ceil(box.maxZ); z++)
                    if (mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() == block) return true;
